...
class CostTracker:
    ...
    MODEL_PRICING = {
        ...
        "us.meta.llama3-2-11b-instruct-v1:0": {
            "input": 0.00035,  # $0.35 per 1M input tokens
            "output": 0.00035  # $0.35 per 1M output tokens
        },
    ...
    def generate_report(self):
        ...
        report.append("PRICING REFERENCE (per 1M tokens):")
        report.append("-" * 50)
        for model_id, pricing in self.MODEL_PRICING.items():
            model_name = model_id.split(".")[-1].replace("-v1:0", "")
            report.append(f"{model_name}:")
            report.append(f"  Input: ${pricing['input']*1000:.2f}")
